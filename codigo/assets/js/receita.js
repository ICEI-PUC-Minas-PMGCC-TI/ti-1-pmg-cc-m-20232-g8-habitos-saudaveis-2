async function popularDiv() {
    ...
    dados.forEach(dado => {
        ...
        let novaDiv = `<div id="teste" class="d-fluid">
            <h4>${dado.nomerec}</h4>
            <div class="row">
                <div class="col-4">
...
async function mostarModalDeIngredientes(id) {
    ...
    dados.forEach(dado => {
        if (dado.id == id) {
            let ingredientesFormatados = dado.ingredientes.split(',').map(ingrediente => `<li>${ingrediente.trim()}</li>`).join('');
            let preparoFormatado = dado.preparo.split(/\d+\-/).filter(item => item.trim() !== '').map(passo => `<li>${passo.trim()}</li>`).join('');
